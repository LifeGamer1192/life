...
// Animals are drawn larger than their cell so the silhouettes are easy to
// read on a grid this dense; the simulation logic still treats them as
// occupying exactly one cell.
const ANIMAL_DISPLAY_SCALE = 1.8;
...
const VERSION = 10; // Seasonal cycle + visual polish (sprites, animal direction)

// ---------- Seasonal climate cycle ----------
// The base temperature[] grid is built once at start. A global seasonal
// offset oscillates over time and is added to base temperature wherever
// fitness (and the canvas background color) is computed.
let SEASON_PERIOD    = 800; // steps per full year (4 seasons of 200 each)
let SEASON_AMPLITUDE = 0.15; // peak +/- offset added to base temperature

function currentSeasonOffset() {
  if (SEASON_AMPLITUDE === 0 || SEASON_PERIOD <= 0) return 0;
  const phase = (stepCount / SEASON_PERIOD) * 2 * Math.PI;
  return SEASON_AMPLITUDE * Math.sin(phase);
}
function currentSeasonName() {
  if (SEASON_PERIOD <= 0) return 'Steady';
  const phase = ((stepCount % SEASON_PERIOD) + SEASON_PERIOD) % SEASON_PERIOD / SEASON_PERIOD;
...
const DEFAULT_PLANT_PARAMS = [
  ...
  { name: 'Moss',  spriteFile: 'plant_moss.png',  color: [130, 200, 210], optTemp: 0.15, optHum: 0.85, growth: 0.75, decay: 1.0, maxBio: 50,  spreadThresh: 20 },
];
...
function stepAnimals() {
  ...
  for (const a of order) {
    ...
    // Remember the last move direction so we can draw a "facing" indicator.
    if (mx !== 0 || my !== 0) {
      a.lastDx = mx;
      a.lastDy = my;
    }
...
function fitness(sp, i) {
  // Effective temperature includes the seasonal offset (clamped to [0,1]).
  const effT = Math.max(0, Math.min(1, temperature[i] + currentSeasonOffset()));
  const dt = Math.abs(effT - PLANT_SPECIES[sp].optTemp);
...
const elSeason  = document.getElementById('status-season');
...
const elMiniGraph    = document.getElementById('mini-graph');
const miniGraphCtx   = elMiniGraph ? elMiniGraph.getContext('2d') : null;
const MINI_GRAPH_WINDOW = 120; // last N samples to plot
...
function initStatusPanel() {
  // The "v" prefix is in the HTML; only the number goes here.
  elVersion.textContent = String(VERSION);

  // Plants section (compact inline pills; source label is now in the editor)
  elPlants.innerHTML = '';
...
function updateStatusPanel() {
  ...
  if (elSeason) {
    const off = currentSeasonOffset();
    const sign = off >= 0 ? '+' : '−'; // minus sign for negative
    elSeason.textContent = `${currentSeasonName()} ${sign}${Math.abs(off).toFixed(2)}`;
  }
  for (let sp = 0; sp < NUM_PLANT_SPECIES; sp++) {
...
// Render two-series mini graph: Plants total (green) and Animals total
// (white-pink). Each series is normalized to its own max within the window,
// so animals (small absolute counts) are still visible alongside plants.
function updateMiniGraph() {
  const c = miniGraphCtx;
  if (!c) return;
  const w = elMiniGraph.width;
  const h = elMiniGraph.height;
  c.clearRect(0, 0, w, h);
  c.fillStyle = 'rgba(0, 0, 0, 0.25)';
  c.fillRect(0, 0, w, h);
  const slice = aliveHistory.slice(-MINI_GRAPH_WINDOW);
  const n = slice.length;
  if (n === 0) return;

  const plantTotals  = slice.map(e => e.plantCounts.reduce((a, b) => a + b, 0));
  const animalTotals = slice.map(e => e.animalCounts.reduce((a, b) => a + b, 0));
  const plantMax  = Math.max(1, ...plantTotals);
  const animalMax = Math.max(1, ...animalTotals);

  const padX = 4;
  const padTop = 16; // leave a strip for the labels
  const padBot = 4;
  const gw = w - padX * 2;
  const gh = h - padTop - padBot;
  const xs = (i) => padX + (n <= 1 ? 0 : (i / (n - 1)) * gw);
  const ysP = (v) => padTop + (1 - v / plantMax)  * gh;
  const ysA = (v) => padTop + (1 - v / animalMax) * gh;

  // Plants series (yellow-green)
  c.lineWidth = 1.5;
  c.strokeStyle = 'rgb(170, 220, 90)';
  c.beginPath();
  for (let i = 0; i < n; i++) {
    const x = xs(i), y = ysP(plantTotals[i]);
    if (i === 0) c.moveTo(x, y);
    else c.lineTo(x, y);
  }
  c.stroke();

  // Animals series (warm white)
  c.strokeStyle = 'rgb(255, 170, 170)';
  c.beginPath();
  for (let i = 0; i < n; i++) {
    const x = xs(i), y = ysA(animalTotals[i]);
    if (i === 0) c.moveTo(x, y);
    else c.lineTo(x, y);
  }
  c.stroke();

  // Header labels: latest values for both series
  c.font = '11px sans-serif';
  c.textBaseline = 'top';
  c.textAlign = 'left';
  c.fillStyle = 'rgb(170, 220, 90)';
  c.fillText(`Plants ${plantTotals[n - 1]}`, padX, 2);
  c.fillStyle = 'rgb(255, 170, 170)';
  c.textAlign = 'right';
  c.fillText(`Animals ${animalTotals[n - 1]}`, w - padX, 2);
  c.textAlign = 'start';
}
...
function initDropPanel() {
  ...
  // the parameter editor); fall back to per-species sensible defaults.
  const prev = dropCounts.slice();
  dropCounts = [];
  for (let sp = 0; sp < NUM_ANIMAL_SPECIES; sp++) {
    const def = ANIMAL_SPECIES[sp];
    const fallback = (def && def.name === 'Herbivore') ? 30 : 10;
    dropCounts.push(prev[sp] != null ? prev[sp] : fallback);
  }
  for (let sp = 0; sp < NUM_ANIMAL_SPECIES; sp++) {
    const def = ANIMAL_SPECIES[sp];
    const row = document.createElement('div');
    ...
    row.innerHTML = `
      <span class="drop-max-cell"><span class="drop-max-label">cap</span><input type="number" class="drop-max-input" min="1" value="${def.maxPopulation}"></span>
    ...
    const maxInput = row.querySelector('.drop-max-input');
    ...
    // Inline maxPopulation editor: writes back to ANIMAL_SPECIES so the
    // simulation reacts immediately. The full parameter editor stays in sync
    // because renderEditor reads from the same array.
    maxInput.addEventListener('change', () => {
      const n = parseInt(maxInput.value, 10);
      if (Number.isFinite(n) && n >= 1) {
        ANIMAL_SPECIES[sp].maxPopulation = n;
      } else {
        maxInput.value = ANIMAL_SPECIES[sp].maxPopulation;
      }
    });
  }
}
...
function renderEditor() {
  ...
  const pSrc = document.getElementById('editor-plants-source');
  if (pSrc) pSrc.textContent = `Source: ${PLANT_PARAMS_SOURCE}`;
  ...
  const aSrc = document.getElementById('editor-animals-source');
  if (aSrc) aSrc.textContent = `Source: ${ANIMAL_PARAMS_SOURCE}`;
  // Plant rules (global)
  ...
  const simEntries = [
    ...
    { key: 'SEASON_PERIOD',    label: 'SEASON_PERIOD',    type: 'int',   step: 50,   min: 0, value: SEASON_PERIOD },
    { key: 'SEASON_AMPLITUDE', label: 'SEASON_AMPLITUDE', type: 'float', step: 0.01, min: 0, max: 0.5, value: SEASON_AMPLITUDE },
  ];
...
function applyEditor() {
  ...
  document.querySelectorAll('#editor-sim input[data-key]').forEach(inp => {
    ...
    switch (inp.dataset.key) {
      ...
      case 'SEASON_PERIOD':    SEASON_PERIOD    = Math.max(0, v); break;
      case 'SEASON_AMPLITUDE': SEASON_AMPLITUDE = Math.max(0, v); break;
    }
  });
...
function draw() {
  ...
  // 1. Environment (background) -- temperature is offset by the current season
  const seasonOff = currentSeasonOffset();
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < M; x++) {
      const i = idx(y, x);
      const effT = Math.max(0, Math.min(1, temperature[i] + seasonOff));
      ctx.fillStyle = envColor(effT, humidity[i]);
  ...
  // 3. Animals: draw per-species sprite at ANIMAL_DISPLAY_SCALE times the
  //    cell size, centered on the cell. Flip horizontally if the last move
  //    was leftward so the silhouette points where the animal is heading.
  const animSize = cell * ANIMAL_DISPLAY_SCALE;
  const animOffset = (animSize - cell) / 2;
  for (const a of animals) {
    const def = ANIMAL_SPECIES[a.species];
    const px = a.x * cell - animOffset;
    const py = a.y * cell - animOffset;
    const flip = (a.lastDx | 0) < 0;
    if (animalSpritesLoaded[a.species]) {
      if (flip) {
        ctx.save();
        ctx.translate(px + animSize, py);
        ctx.scale(-1, 1);
        ctx.drawImage(animalSprites[a.species], 0, 0, animSize, animSize);
        ctx.restore();
      } else {
        ctx.drawImage(animalSprites[a.species], px, py, animSize, animSize);
      }
    } else {
      // Fallback shape: filled circle at the larger size.
      ctx.beginPath();
      ctx.arc(px + animSize / 2, py + animSize / 2, animSize / 2.5, 0, Math.PI * 2);
...
async function main() {
  ...
  // Climate-forcing buttons next to the temperature / humidity legend bars.
  // Each click shifts every cell's base temperature or humidity by the data-
  // delta amount (clamped to [0, 1]). Plants react on the next step via
  // fitness, and the background colour updates on the next draw.
  document.querySelectorAll('.env-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const axis = btn.dataset.axis;
      const delta = parseFloat(btn.dataset.delta);
      if (!Number.isFinite(delta)) return;
      const arr = (axis === 'temp') ? temperature
                : (axis === 'hum')  ? humidity : null;
      if (!arr) return;
      for (let i = 0; i < arr.length; i++) {
        arr[i] = Math.max(0, Math.min(1, arr[i] + delta));
      }
