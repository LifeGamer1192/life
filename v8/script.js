...
const VERSION = 8; // Stage 3+: delayed species spawning + flash effect + speed control
...
function stepPlants() {
  ...
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < M; x++) {
      ...
      if (plantBiomass[i] > def.spreadThresh && Math.random() < PLANT_SPREAD_PROB) {
...
// Flash the legend row of an animal species when it is first introduced.
function flashAnimalRow(speciesIdx) {
  const countEl = elAnimalCounts[speciesIdx];
  if (!countEl) return;
  const row = countEl.closest('.legend-row');
  if (!row) return;
