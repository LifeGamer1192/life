...
const VERSION = 6; // Stage 2+: herbivore reproduction + downloadable run log
...
function countsByAnimalSpecies() {
  ...
  for (const a of animals) counts[a.species]++;
  return counts;
}
...
function stepAnimals() {
  ...
  for (const a of animals) {
    ...
    // 1. Scan vision (including own cell) for plant cell with the most biomass.
    let bestY = a.y, bestX = a.x, bestBio = 0;
    ...
    // 2. Move one cell toward the chosen target; if nothing in sight, random walk.
    let mx = 0, my = 0;
    if (bestBio > 0) {
      mx = Math.sign(torDelta(a.x, bestX, M));
      my = Math.sign(torDelta(a.y, bestY, N));
    } else {
    ...
    // 3. Eat at new location if there is a plant there.
    const i = idx(a.y, a.x);
    if (plantSpecies[i] >= 0) {
